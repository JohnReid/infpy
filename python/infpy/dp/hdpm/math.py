...
def _approx_2nd_order(f_E_plus_n, f2_E_plus_n, P_plus_n, V_plus_n):
    ...
    result = numpy.asarray(
        P_plus_n * (f_E_plus_n + .5 * V_plus_n * f2_E_plus_n))
...
class BetaDist(object):
    ...
    def __init__(self, a, b):
        ...
        self.H = betaln(a, b) - (a - 1.) * digamma(a) - \
...
class DirichletDist(object):
    ...
    def __init__(self, a):
        ...
        self.H = gammaln(a).sum() - gammaln(self.a_sum) + \
            ((a - 1.) * self.G).sum()
        "Entropy"
...
def approximate_beta_expectation(a, counts_P_plus, counts_E_plus, counts_V_plus):
    ...
    result = numpy.asarray(counts_P_plus * (digamma(a) -
                                            digamma(term) - counts_V_plus * polygamma(2, term) / 2.))
