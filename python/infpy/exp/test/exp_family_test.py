...
def check_is_close(u, v, tol=1e-4, strong_test=True, dont_test_zeros=False):
    ...
    u = numpy.asarray(u)
    v = numpy.asarray(v)
    assert u.shape == v.shape, 'u and v are not the same shape: %s != %s' % (
        str(u.shape), str(v.shape))
...
class CheckExpFamily(object):
        ...
        for x in self.family._typical_xs:
            ...
            if tuple == type(x):
                ...
                assert check_is_close(
                    x, self.family.x(self.family.T(x))), str(x)
    ...
    def check_thetas(self):
        ...
        for theta in self.family._typical_thetas:
            ...
            assert check_is_close(eta, self.family.eta(
                self.family.theta(eta)), strong_test=False, dont_test_zeros=True), str(eta)
        ...
        for x in self.family._typical_xs:
            for theta in self.family._typical_thetas:
                ...
                assert check_is_close(calculated, truth, strong_test=False, dont_test_zeros=True), \
                    ...
                    str(self.family.__class__), str(
                        x), str(theta), calculated, truth
                )
    ...
    def check_entropy_truth(self):
        ...
        for theta in self.family._typical_thetas:
            ...
            assert check_is_close(calculated, truth), \
                ...
                str(self.family.__class__), str(
                    theta), str(eta), calculated, truth
            )
...
class CheckConjugatePrior(object):
    ...
    def check_log_normalisation_factor_expectation(self, sample_size=1000):
        ...
        for theta in self.conj_prior.prior._typical_thetas:
            ...
            calculated_lnf = self.conj_prior.exp_likelihood_log_normalisation_factor(
                eta)
            samples = self.conj_prior.prior.sample(eta, size=sample_size)
            sampled_lnf = 0.
            for sample in samples:
                sampled_lnf += self.conj_prior.likelihood.A(
                    sample[self.conj_prior.strength_dimension:])
            ...
            for mu_0 in [[0., 0.], [5., -3.]]:
                for nu, S in WishartExpFamily._typical_thetas:
                    ...
                    for T_W in sampled_W:
                        ...
                        precision = kappa_0 * W
                        mu = mvn.x(
                            mvn.sample(eta=mvn.eta((mu_0, precision)), size=1)[0])
...
class HighDMvnTest(unittest.TestCase):
    ...
    def setUp(self):
                    # Create a sequence of exponential families of high
                    # dimension
        self.families = [
