...
def kernel_init_args_helper(
    ...
    if None == dimensions:
        if None == params:
            if None == priors:
                dimensions = 1  # assume dims = 1
            else:
                dimensions = len(priors) - num_extra_params
        else:
...
class Kernel(object):
    ...
    def __init__(
        ...
        if len(params) != len(param_priors):
            raise RuntimeError(
                """Must have same number of priors as parameters""")
        self.params = params
    ...
    def supports(self, p):
        ...
        for p1, prior in zip(p, self.param_priors):
            if None != prior and not prior.supports(p1):
                return False
        return True
